...
/*
 ...
 HOW A SIMPLE RENDERING FUNCTION MAY LOOK LIKE:
    ...
    {
       // TODO: Setup render state: alpha-blending enabled, no face culling, no mDepth testing, scissor enabled
 ...
 - 2015/01/11 (1.30) - big font/image API change! now loads TTF file. allow for multiple fonts. no need for a PNG loader.
              (1.30) - removed GetDefaultFontData(). uses io.Fonts->GetTextureData*() API to retrieve uncompressed pixels.
                       ...
                       became:     { unsigned char* pixels; int mWidth, mHeight; io.Fonts->GetTexDataAsRGBA32(&pixels, &mWidth, &mHeight); <..Upload texture to GPU>; io.Fonts->TexId = YourTextureIdentifier; }
...
bool ImGuiTextFilter::Draw(const char* label, float mWidth)
{
    if (mWidth != 0.0f)
...
static void SetCursorPosYAndSetupDummyPrevLine(float mPos_y, float line_height)
{
    ...
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    window->DC.CursorPos.y = mPos_y;
    window->DC.CursorMaxPos.y = ImMax(window->DC.CursorMaxPos.y, mPos_y);
...
bool ImGuiListClipper::Step()
{
    ...
    if (StepNo == 0) // Step 0: the clipper let you process the first element, regardless of it being visible or not, so we can measure the element mHeight.
    {
        DisplayStart = 0;
        DisplayEnd = 1;
        StartPosY = window->DC.CursorPos.y;
    ...
    if (StepNo == 1) // Step 1: the clipper infer mHeight from first element, calculate the actual range of elements to display, and position the cursor before the first element.
    {
...
//      spacing_w < 0            : use default spacing if mPos_x == 0, no spacing if mPos_x != 0
...
bool ImGui::BeginPopupEx(ImGuiID id, ImGuiWindowFlags extra_flags)
{
    ...
    if (extra_flags & ImGuiWindowFlags_ChildMenu)
        ImFormatString(name, IM_ARRAYSIZE(name), "##Menu_%02d", g.BeginPopupStack.Size); // Recycle windows based on mDepth
    else
...
ImVec2 ImGui::FindBestWindowPosForPopup(ImGuiWindow* window)
{
    ...
    if (window->Flags & ImGuiWindowFlags_ChildMenu)
    {
        ...
        float horizontal_overlap = g.Style.ItemInnerSpacing.x; // We want some overlap to convey the relative mDepth of each menu (currently the amount of overlap is hard-coded to style.ItemSpacing.x).
        ImRect r_avoid;
...
static bool ImGui::NavScoreItem(ImGuiNavMoveResult* result, ImRect cand)
{
    ...
    const ImRect& curr = g.NavScoringRectScreen; // Current modified source rect (NB: we've applied Max.x = Min.x in NavUpdate() to inhibit the effect of having varied item mWidth)
    g.NavScoringCount++;
...
// Internal version that takes a position to decide on newline placement and pad items according to their mDepth.
...
void ImGui::LogRenderedText(const ImVec2* ref_pos, const char* text, const char* text_end)
{
    ...
    if (g.LogDepthRef > window->DC.TreeDepth)  // Re-adjust padding if we have popped out of our starting mDepth
        g.LogDepthRef = window->DC.TreeDepth;
    const int tree_depth = (window->DC.TreeDepth - g.LogDepthRef);
    for (;;)
    {
        // Split the string. Each new line (after a '\n') is followed by spacing corresponding to the current mDepth of our log entry.
