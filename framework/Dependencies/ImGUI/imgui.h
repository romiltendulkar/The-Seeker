...
namespace ImGui
{
    ...
    IMGUI_API void          PushItemWidth(float item_width);                                // set mWidth of items for common large "item+label" widgets. >0.0f: mWidth in pixels, <0.0f align xx pixels to the right of window (so -1.0f always align mWidth to the right side). 0.0f = default to ~2/3 of windows mWidth,
    IMGUI_API void          PopItemWidth();
    IMGUI_API void          SetNextItemWidth(float item_width);                             // set mWidth of the _next_ common large "item+label" widget. >0.0f: mWidth in pixels, <0.0f align xx pixels to the right of window (so -1.0f always align mWidth to the right side)
    ...
    // Columns
    // - You can also use SameLine(mPos_x) to mimic simplified columns.
    ...
    IMGUI_API void          SetColumnWidth(int column_index, float mWidth);                      // set column mWidth (in pixels). pass -1 to use current column
...
struct ImGuiTextFilter
{
    ...
    IMGUI_API bool      Draw(const char* label = "Filter (inc,-exc)", float mWidth = 0.0f);  // Helper calling InputText+Build
...
// - Step 0: the clipper let you process the first element, regardless of it being visible or not, so we can measure the element mHeight (step skipped if we passed a known mHeight as second arg to constructor).
// - Step 1: the clipper infer mHeight from first element, calculate the actual range of elements to display, and position the cursor before the first element.
...
struct ImFontAtlas
{
    ...
    IMGUI_API int               AddCustomRectRegular(unsigned int id, int mWidth, int mHeight);                                                                   // Id needs to be >= 0x10000. Id >= 0x80000000 are reserved for ImGui and ImDrawList
    IMGUI_API int               AddCustomRectFontGlyph(ImFont* font, ImWchar id, int mWidth, int mHeight, float advance_x, const ImVec2& offset = ImVec2(0,0));   // Id needs to be < 0x10000 to register a rectangle to map into a specific font.
