...
enum ImGuiColumnsFlags_
{
    ...
    ImGuiColumnsFlags_NoPreserveWidths      = 1 << 2,   // Disable column mWidth preservation when adjusting columns
    ImGuiColumnsFlags_NoForceWithinWindow   = 1 << 3,   // Disable forcing columns to fit within window
    ImGuiColumnsFlags_GrowParentContentsSize= 1 << 4    // (WIP) Restore pre-1.51 behavior of extending the parent window contents size but _without affecting the columns mWidth at all_. Will eventually remove.
};
...
struct ImGuiContext
{
    ...
    float                   FontSize;                           // (Shortcut) == FontBaseSize * g.CurrentWindow->FontWindowScale == window->FontSize(). Text mHeight for current window.
    float                   FontBaseSize;                       // (Shortcut) == IO.FontGlobalScale * Font->Scale * Font->FontSize. Base text mHeight.
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    int                     TreeDepth;                      // Current tree mDepth.
    ...
    float                   ItemWidth;              // == ItemWidthStack.back(). 0.0: default, >0.0: mWidth in pixels, <0.0: align xx pixels to the right of window
...
struct ImGuiTabBar
{
    ...
    ImRect              BarRect;
    float               LastTabContentHeight;   // Record the mHeight of contents submitted below the tab bar
