...
Gate::Gate() : Entity(ENTITY_TYPE::GATE)
{
	mTimer = 0;
	mTimerLimit = 2;
}
...
void Gate::Serialize(Json::Value root, unsigned int position)
{
	...
	(*MemoryManager::GetInstance()->ObjectPool)[position].pEn->mEntityName = "Gate";
	key = position;
	this->complete = true;
	this->runtime = false;
}
...
void Gate::HandleEvent(Event* pEventToHandle)
{
	if (pEventToHandle)
	{
